      ...
      <div className="atelier-workspace">
        ...
        <aside className="atelier-info" aria-live="polite" aria-label="Anatomy information">
          ...
          <div className="atelier-content-badges" aria-label="Content review and source">
            <span><BadgeCheck size={14} /> Sourced educational anatomy</span>
          ...
          <section className="atelier-relationships" aria-label={`${selectedOrgan.name} relationships`}>
            ...
            {selectedRelationship ? <div className="atelier-relationship-answer"><b>{selectedRelationship.kind}</b><p>{selectedRelationship.summary}</p>{selectedRelationshipReference && <a href={selectedRelationshipReference.href} rel="noreferrer" target="_blank">Reference source: {selectedRelationshipReference.label} <ExternalLink aria-hidden="true" size={12} /></a>}</div> : <p className="atelier-relationship-empty">This pair is not mapped in this specimen yet. Try a highlighted connection from the current specimen.</p>}
          ...
          <p className="atelier-content-status">Content level: concise orientation. More detailed, sourced structure notes will be added as each model is mapped.</p>
        </aside>
      </div>
    </main>
  );
